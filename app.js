...
makePrinter(function(print){
    startServer(app, config, print, function(err){
        if (err) throw err;
...
function makePrinter(callback){
    ...
    serialPort.on('open', function(err){
        ...
        */
        ...
        printer.on('ready', function(){ callback(print); });

    });

}

function startServer(app, config, print, callback){
    ...
    var server = app.listen(config.ports.printer, function(){
        ...
        require('./printOuts/route')(app, config, print);
